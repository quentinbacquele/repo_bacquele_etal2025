...
# SES count columns you want to model (8 syndromes)
...
dat <- dat4 %>%
  arrange(lon, lat) %>%
  mutate(idx = row_number()) %>%   # useful if you later run BYM2
...
# Mesh & SPDE (copying the priors from your reference code)
mesh <- inla.mesh.2d(
  loc = coords,
  max.edge = c(2, 4),
  offset   = c(1, 2),
